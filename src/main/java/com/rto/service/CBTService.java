...
public class CBTService implements IService {
    ...
    public CBTResult evaluateTest(String userId, Map<String, String> answers, int totalQuestions) {
        ...
        // Use Factory Pattern to get applicable configuration (OCP)
        List<CBTResult> history = getTestHistory(userId);
        int previousAttempts = history != null ? history.size() : 0;
        ICBTConfig config = CBTConfigFactory.getConfig(previousAttempts);

        // Use provided totalQuestions (handles unanswered questions)
        if (totalQuestions <= 0) {
            totalQuestions = config.getQuestionsPerTest();
        }

        // Calculate score percentage and pass status using the configuration
        int scorePercentage = totalQuestions > 0 ? (correctCount * 100) / totalQuestions : 0;
        boolean passed = config.isPassed(correctCount, totalQuestions);
